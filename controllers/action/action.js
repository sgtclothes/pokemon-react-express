...
exports.logJSON = {
    ...
    generateJSON: (data) => {
        try {
            ...
            let response = { status: "success", message: "Successfully created json log" };
    ...
    getJSON: () => {
        try {
            let dateFileName = moment().format("DD-MM-YYYY");
            let json = JSON.parse(fs.readFileSync(path + "/" + dateFileName + ".json", "utf8"));
            return json;
        } catch (error) {
            console.log({ message: error.message });
        }
    ...
    updates: {
        login: (data) => {
            try {
                ...
                let json = JSON.stringify(dataJSON);
                let response = { status: "success", message: "Successfully updated json log" };
                fs.writeFile(path + "/" + dateFileName + ".json", json, "utf8", () => {
                    console.log(response);
                });
                return json;
        ...
        search: (data) => {
            try {
                let key = config.prefix + "_" + data.loginInfo.us_id + "_" + data.loginInfo.us_username;
                let dataJSON = this.logJSON.getJSON();
                let datetime = moment().format();
                let dateFileName = moment().format("DD-MM-YYYY");
                let log = { type: "search", action: "search '" + data.searchTerm + "'", time: datetime };
                if (!dataJSON.logs.hasOwnProperty(key)) {
                    dataJSON.logs[key] = [log];
                } else {
