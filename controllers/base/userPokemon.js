...
let additionalMethods = {
  ...
  generateResponse: (key, result, configuration, type) => {
    ...
    let response = {
      ...
      message:
        ...
        (result.check ? " " : " don't ") +
        "get " +
        additionalMethods.camelCaseToSentence(
          configuration.config_data[key].target
        ) +
        "!",
    };
    return response;
  ...
  getAllMyPokemon: async (req, res) => {
    try {
      let { token } = req.body;
      let loginInfo = baseToken
        .methods()
        .verifyToken(token, config.secret, res);
  ...
  catchingPokemon: async (req, res) => {
    try {
      ...
      const configuration = await additionalMethods.getConfigurationByType(
        "logic"
      );
      let loginInfo = baseToken
        .methods()
      ...
      let result =
        number.methods[configuration.config_data[key].target].execute("random");
      res
        .status(200)
        .send(additionalMethods.generateResponse(key, result, configuration));
    } catch (error) {
      res.status(500).send({ message: error.message });
    }
  },
  releasePokemon: async (req, res) => {
    try {
      ...
      let result =
        number.methods[configuration.config_data[key].target].execute("random");
      let response = additionalMethods.generateResponse(
        key,
        result,
        configuration
      );
  ...
  renamePokemon: async (req, res) => {
    try {
      ...
      let result = number.methods[
        configuration.config_data[key].target
      ].execute("row", previousNumber);
